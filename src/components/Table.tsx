...
export function Table({room,session,busy,connection,action,report}:Props) {
  ...
  return <main className="table-page">
    ...
    {waiting?<section className="lobby-actions"><div className="lobby-main-buttons"><button className={`button ${me.ready?'secondary':'primary'}`} disabled={!canAct} onClick={()=>action({type:'ready',ready:!me.ready})}>{me.ready?<><X size={17}/>取消准备</>:<><Check size={18}/>我准备好了</>}</button>{host&&<button className="button primary" disabled={!canAct||!canStart} onClick={()=>action({type:'start'})}><Play size={17}/>开始游戏</button>}</div>{host&&<div className="host-tools"><button className="text-button" disabled={!canAct||room.players.some(p=>p.ready)} onClick={()=>action({type:'shuffleTeams'})}><Shuffle size={15}/>随机组队</button><button className="text-button" disabled={!canAct} onClick={()=>{setSwapping(!swapping);setSwapFrom(null);}}><RefreshCw size={14}/>{swapping?'结束换座':'调整座位'}</button></div>}<p className="fine-print">{computerRoom?(host?'当前真人均准备后即可开始，空位自动由电脑补齐。换座前请先取消准备。':'等待房主开始；还可以分享房间号邀请真人入座。'):host?'所有人准备后，即可开始。换座前请先取消准备。':'准备后等待房主开始。分享链接，再邀请几位好友。'}</p></section>:<>
