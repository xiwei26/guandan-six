...
export function Table({room,session,busy,connection,action,report}:Props) {
  ...
  const computerRoom=room.mode==='computer';
  const demo=computerRoom||room.players.some(p=>p.bot);
  const realPlayers=room.players.filter(p=>!p.bot);
  const canStart=computerRoom?realPlayers.length>0&&realPlayers.every(p=>p.ready):room.players.length===6&&room.players.every(p=>p.ready);
  ...
  return <main className="table-page">
    <div className="room-toolbar"><button className="text-button" onClick={()=>setLeaveOpen(true)}><ArrowLeft size={17}/><span>返回大厅</span></button><div className="room-id">{computerRoom?'电脑局':'好友房'} <b>{room.roomId}</b><span>{room.rules.rounds==='A'?'打到 A':`${room.rules.rounds} 局`}{room.round>0?` · 第 ${room.round} 局`:''}</span></div><div className="toolbar-tools"><button className="text-button" onClick={share}><Copy size={16}/><span>复制邀请</span></button>{room.rules.allowCounter&&room.round>0&&<button className={`text-button ${counterOpen?'active-tool':''}`} onClick={()=>setCounterOpen(!counterOpen)} aria-pressed={counterOpen}><Calculator size={16}/><span>记牌</span></button>}</div></div>
    <div className="table-score"><div className="team-score A"><span className="team-dot"/>蓝队<strong>{room.teamLevels.A}</strong></div><div className="table-status">{waiting?<><Users size={16}/>{computerRoom?realPlayers.length:room.players.length}/6 {computerRoom?'位真人':'人入座'}</>:<><span className="level-indicator">打 {room.currentLevel}</span><span>{demo?`电脑补位 · ${room.players.filter(p=>p.bot).length} 位电脑`:'三人一队，默契上桌'}</span></>}</div><div className="team-score B"><strong>{room.teamLevels.B}</strong>橙队<span className="team-dot"/></div></div>
    ...
    {waiting?<section className="lobby-actions"><div className="lobby-main-buttons"><button className={`button ${me.ready?'secondary':'primary'}`} disabled={!canAct} onClick={()=>action({type:'ready',ready:!me.ready})}>{me.ready?<><X size={17}/>取消准备</>:<><Check size={18}/>我准备好了</>}</button>{host&&<button className="button primary" disabled={!canAct||!canStart} onClick={()=>action({type:'start'})}><Play size={17}/>开始游戏</button>}</div>{host&&!computerRoom&&<div className="host-tools"><button className="text-button" disabled={!canAct||room.players.some(p=>p.ready)} onClick={()=>action({type:'shuffleTeams'})}><Shuffle size={15}/>随机组队</button><button className="text-button" disabled={!canAct} onClick={()=>{setSwapping(!swapping);setSwapFrom(null);}}><RefreshCw size={14}/>{swapping?'结束换座':'调整座位'}</button></div>}<p className="fine-print">{computerRoom?(host?'当前真人均准备后即可开始，空位自动由电脑补齐。':'等待房主开始；还可以分享房间号邀请真人入座。'):host?'所有人准备后，即可开始。换座前请先取消准备。':'准备后等待房主开始。分享链接，再邀请几位好友。'}</p></section>:<>
